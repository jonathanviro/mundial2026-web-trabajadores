...
function Stepper({
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      <span
        className="w-10 h-8 flex items-center justify-center bg-white/10
...
export default function PredictPage() {
  ...
  const required = isDaily
    ? matches.length || 1
    : phase?.predictions_required || 3;
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="bg-black/40 backdrop-blur-md border-b border-white/10 flex-shrink-0">
        <div className="flex items-center justify-between px-4 md:px-8 py-3">
          <button
            onClick={() =>
              selectedTeam ? setSelectedTeam(null) : setScreen("dashboard")
            }
          ...
          <div className="text-center">
            <div className="text-xs text-[#7a8899] uppercase tracking-wider">
              {isDaily
                ? `Partidos del ${predictionDate || "mañana"}`
                : showTeamGrid
                  ? "Elige un equipo"
                  : selectedTeam || phase?.name}
            </div>
            <div className="text-sm font-bold">
              {isDaily ? `${phase?.name} - Predicción diaria` : phase?.name}
            </div>
          </div>
          <div className="text-right">
            <div className="text-xs text-[#7a8899]">Predicciones</div>
            <div
              className={`text-xl font-black ${done >= required ? "text-green-400" : "text-accent"}`}
            >
              {done} / {required}
            </div>
          </div>
                ...
                <div className="mt-8 w-full max-w-4xl mx-auto">
                  ...
                  {done >= required && (
                    <p className="text-xs text-yellow-500 mb-3 font-medium">
                      ¡Completaste tus predicciones! Ahora predice el campeón
                      del mundo
                    </p>
                  )}
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-center mt-2">
                            <button
                              onClick={() => handleRemovePrediction(p.match_id)}
                ...
                {filteredMatches.map((match) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        boxShadow:
                          "0 8px 32px rgba(0,0,0,.2), inset 0 1px 0 rgba(255,255,255,.06)",
                      }}
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 flex flex-col items-center text-center">
                          ...
                          <div className="text-xs md:text-sm font-bold mt-1 leading-tight mb-2">
                            {match.team_local}
                          </div>
                          {isPredicted && (
                            <Stepper
                        ...
                        <div className="flex flex-col items-center pt-8 flex-shrink-0">
                          {isPredicted ? (
                            <span className="text-lg font-bold text-[#7a8899]">
                              :
                            </span>
                          ) : (
                            <button
            ...
            <button
              onClick={() => {
                if (done < required) {
                  alert(
                    isDaily
                      ? "Debes predecir al menos 1 partido"
                      : `Debes hacer al menos ${required} predicciones`,
                  );
                  return;
                }
                if (!isDaily && !champion) {
